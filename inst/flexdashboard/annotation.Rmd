...
```{r}
renderUI({
  HTML(sprintf("Doc %s/%s<br>doc_id: %s", dashdata$current_row+1, dashdata$total_rows, currentText()$doc_id))
})
tags$br()
...
reactive({
  ...
  if(!is.null(input$ui_tid_input) && file.exists(input$ui_tid_input$datapath)){
    if(tools::file_ext(input$ui_tid_input$name) %in% c("rds", "RDS", "Rds")){
      dashdata$save_as <<- sprintf("crfsuite_annotation_%s.rds", gsub(".rds$|.RDS$", "", input$ui_tid_input$name))
...
```{r}
dataTableOutput(outputId = "ui_taggeddata")
output$ui_taggeddata <- renderDataTable({
  input$ui_next
  if(file.exists(dashdata$save_as)){
    data <- readRDS(dashdata$save_as)
    data <- rbindlist(data)
    data <- setDF(data)
    data <- data[, c("doc_id", "start", "end", "category", "selection")]
  }else{
    data <- data.frame(doc_id = character(), start = integer(), end = integer(), category = character(), selection = character(), stringsAsFactors = FALSE)
  }
  data
